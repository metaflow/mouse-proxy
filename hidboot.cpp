...
void MouseReportParser::Parse(USBHID *hid, bool is_rpt_id, uint8_t len, uint8_t *buf) {
  ...
  Debug(is_rpt_id, len, buf);
        ...
        int16_t diff = previousButtonsState ^ buf[0];
        for (uint16_t mask = 1; mask != (1u << 8); mask <<= 1) {
          if (diff & mask) {
            if (previousButtonsState & mask) {
              OnButtonUp(mask);
            } else {
              OnButtonDown(mask);
            }
        ...
        // (actually not: when you press a button coords are reported as well).
        // Why not just pass the x/y values to simplify things??
        if(pmi->dX || pmi->dY || pmi->wheel) {
                OnMouseMove(pmi);
                // Future:
                // event = true;
        }
